...
/* "@(#)xruSTABS.c Nov  7 22:55:32 2022" */
                ...
                X2C_BOOLEAN * Error)
{
   ...
   Read(Error, (X2C_LOC *)ELF_Header, sizeof(struct xruSTABS_ELF_HEADER)/1u,
                0L, 52L);
   if (*Error) return;
   if ((((X2C_CARD32)ELF_Header->e_type!=2UL || (X2C_CARD32)
                ELF_Header->e_shentsize<40UL) || ELF_Header->e_shoff==0UL)
                || (X2C_CARD32)ELF_Header->e_shstrndx==0UL) {
      *Error = 1;
      return;
   }
   Read(Error, (X2C_LOC *) &SectionHeader,
                ...
                ELF_Header->e_shstrndx*(X2C_CARD32)ELF_Header->e_shentsize),
                40L);
   if (*Error) return;
   *SHStrTabOffset = SectionHeader.sh_offset;
   tmp = (X2C_CARD16)((X2C_CARD32)ELF_Header->e_shnum-1UL);
   i = 1U;
   if (i<=tmp) for (;; i++) {
      Read(Error, (X2C_LOC *) &SectionHeader,
                sizeof(struct xruSTABS_SECTION_HEADER)/1u,
                (X2C_INT32)(ELF_Header->e_shoff+(X2C_CARD32)i*(X2C_CARD32)
                ELF_Header->e_shentsize), 40L);
      if (*Error) return;
